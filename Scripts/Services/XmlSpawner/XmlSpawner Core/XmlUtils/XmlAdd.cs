...
namespace Server.Mobiles
{
    public class XmlSpawnerDefaults
    {
        ...
        {
            if (defs == null) return;
            defs.MinDelay = TimeSpan.FromMinutes(0);
            defs.MaxDelay = TimeSpan.FromMinutes(5);
            ...
            if (defs.QuantityList != null) Array.Clear(defs.QuantityList, 0, defs.QuantityList.Length);
    ...
    public class XmlAddGump : Gump
    {
        ...
        public XmlAddGump(Mobile from, Point3D startloc, Map startmap, bool firststart, bool extension, int gumpx, int gumpy) : base(gumpx, gumpy)
        {
            ...
            if (defs.QuantityList == null)
            {
                defs.QuantityList = Enumerable.Repeat(1, MaxEntries).ToArray();
            }
            ...
            {
                AddBackground(0, 0, 560, 500, 5054);
                AddAlphaRegion(0, 0, 560, 500);
            }
            else
            {
            ...
            if (defs.ShowExtension)
            {
                AddButton(520, y + 5, 0x15E3, 0x15E7, 200, GumpButtonType.Reply, 0);
            }
            else
            {
            ...
            if (defs.ShowExtension)
            {
                ...
                {
                    int xpos = (int)(i / MaxEntriesPerColumn) * 175;
                    int ypos = (i % MaxEntriesPerColumn) * 22 + 30;


                    // background for search results area
                    AddImageTiled(xpos + 205, ypos, 114, 23, 0x52);

                    //BG for quantity
                    AddImageTiled(xpos + 320, ypos + 1, 20, 21, 0xBBC);
                    ...
                    if (i == defs.CategorySelectionIndex)
                    {
                        ...
                        AddImageTiled(xpos + 206, ypos + 1, 113, 21, 0xBBC);
                    ...
                    string quantity = "1";
                    if (defs.QuantityList != null && i < defs.QuantityList.Length)
                        quantity = defs.QuantityList[i].ToString();
                    // display the name
                    ...
                    //Quantity
                    AddTextEntry(xpos + 320, ypos + 1, 20, 21, texthue, 3000 + i, quantity);

                    // display the selection button
                    AddButton(xpos + 340, ypos + 2, (sel ? 0xD3 : 0xD2), (sel ? 0xD2 : 0xD3), 4000 + i, GumpButtonType.Reply, 0);

                    // display the info button
                    AddButton(xpos + 360, ypos + 2, 0x15E1, 0x15E5, 5000 + i, GumpButtonType.Reply, 0);
                }
            }
        }
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                for (int i = 0; i < MaxEntries; i++)
                {
                        ...
                        defs.NameList != null && i < defs.NameList.Length && defs.NameList[i] != null && defs.NameList[i].Length > 0)
                    {
                        if (defs.QuantityList != null && i < defs.QuantityList.Length)
                        {
                            spawner.AddSpawnQuantity(defs.NameList[i], defs.QuantityList[i]);
                        }
                        else
                        {
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (defs.ShowExtension)
            {
                for (int i = 0; i < MaxEntries; i++)
                {
                    ...
                    tr = info.GetTextEntry(3000 + i);
                    if (defs.QuantityList != null && i < defs.QuantityList.Length && tr != null && int.TryParse(tr.Text, out int q))
                    {
                        defs.QuantityList[i] = q;
                    }
