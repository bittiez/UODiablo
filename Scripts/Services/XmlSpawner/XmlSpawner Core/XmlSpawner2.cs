...
namespace Server.Mobiles
{
    public class XmlSpawner : Item, ISpawner
    {
		...
		*/
        ...
        public void AddSpawnQuantity(string name, int quantity)
        {
            if (!string.IsNullOrEmpty(name))
            {
                ...
                if (type != null)
                    m_SpawnObjects.Add(new XmlSpawner.SpawnObject(str, quantity));
                else
                {
                    // check for special keywords
                    if (typestr != null && (BaseXmlSpawner.IsTypeOrItemKeyword(typestr) || typestr.IndexOf("{") != -1 || typestr.StartsWith("*") || typestr.StartsWith("#")))
		...
		*/
        ...
        public void SmartRespawn()
        {
            ...
            //PublicOverheadMessage(MessageType.System, 32, false, "Spawner should have respawned");
        }
