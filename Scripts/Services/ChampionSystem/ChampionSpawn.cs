...
namespace Server.Engines.CannedEvil
{
    public class ChampionSpawn : Item
    {
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public double WavesMod { get; set; } = 1.0;
        ...
        public int MaxKills
        {
            ...
            {
                int l = Level;
                return (int)(ChampionSystem.MaxKillsForLevel(l) * KillsMod);
            }
        }
                    ...
                    if (m.Deleted)
                    {
                        ...
                        if (killer is PlayerMobile)
                        {
                            ...
                            if (Utility.RandomDouble() < ChampionSystem.ScrollChance)
                            {
                                if (Map == Map.Normal)
                                {
                                    ScrollOrTransend(killer, ChampionSystem.ScrollChance, 5, 10);
                                }
                                else if (Map == Map.Nightmare)
                                {
                                    ScrollOrTransend(killer, ChampionSystem.ScrollChance + 0.001, 5, 15);
                                }
                                else if (Map == Map.Hell)
                                {
                                    ScrollOrTransend(killer, ChampionSystem.ScrollChance + 0.002, 5, 20);
                                }
                                else if (Map == Map.Dungeons)
                                {
                                    ScrollOrTransend(killer, ChampionSystem.ScrollChance, 5, 20);
                                }
                            }
                            #endregion
        ...
        private void ScrollOrTransend(Mobile killer, double scrollChance, int min = 5, int max = 5)
        {
            if (Utility.RandomDouble() < scrollChance)
            {
                ...
                {
                    PowerScroll PS = PowerScroll.CreateRandomNoCraft(min, max);
